...
gg <- ggplot(observations_per_id, aes(x = num_observations)) +
      ...
      labs(title = "Distribution of CALHIV by the Number of Viral Load Tests",
           subtitle = "47% of CALHIV have had 2 tests and 85% had 2 to 3 tests",
...
gg <- ggplot(observations_per_id, aes(x = num_observations)) +
      geom_histogram(aes(y = ..count.., 
                         fill = ifelse(num_observations == 2, "grey", hw_viking)), 
                     binwidth = binwidth_value, 
                     color = grey10k) +
      geom_text(
...
ggplot(parenthood_counts, aes(x = parenthood_status_combined, y = n, fill = parenthood_status_combined, label = scales::percent(percentage / 100))) +
 ...
 labs(x = "Parenthood Status", y = "# CALHIV", title = "Parenthood Status for Intervention Participants", subtitle = "67% of the participants had biological parents as caregivers") +
  si_style_ygrid()+
  theme(axis.text.x = element_text(angle = 0, hjust = 1), # Rotate x labels for readability
...
ggplot(count_data, aes(x = time, y = count, fill = intervention)) +
  ...
  geom_text(aes(label = count), position = position_dodge(width = 0.9), vjust = 1.3) +  # Add count labels
...
count_data %>% 
  mutate(viral_load = factor(
    viral_load, levels = c("unsuppressed", "suppressed"), ordered = T)
  ) %>% 
  arrange(viral_load) %>% # force the suppression bar at the bottom
  ggplot(aes(x = time, y = count, fill = viral_load)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(data = count_data %>% filter(viral_load == "suppressed"),
            aes(label = count), vjust = 1.3) +  # Add count labels
  facet_wrap(~ intervention, scales = "free_y", nrow = 1) +
  scale_fill_manual(values = c("unsuppressed" = grey10k, "suppressed" = "#9e94e0"), 
                    labels = c("Unsuppressed", "Suppressed")) +
  scale_x_discrete(labels = c("Start", "Midpoint", "End")) +
  si_style_ygrid() +
  theme(
    panel.grid.major = element_blank(),
  ...
  labs(
    x = "",
    y = "", # CALHIV",
    title = "# of Patient with VLS in the Treatment Group improved over time",
    subtitle = "Viral Suppression consitently increased for the participants with 3 or more viral load tests in the group",  # Subtitle added here
    caption = " Note:Contol cohort was drawn from CALHIV who did not enrol in the intervetion at the site,
                            Source: PCM 2024 data"
  ) 
...
```{r}
  ...
  # Next, create the diverging stacked bar chart
df_vl_age <- age_timepoint_summary %>% 
  pivot_longer(cols = ends_with("pressed"), names_to = "vl_status") %>% 
  mutate(vl_status = factor(vl_status, levels = c("unsuppressed", "suppressed"), ordered = T)) %>% 
  arrange(vl_status) 
...
df_vl_age %>% 
  ggplot(aes(x = age, y = value, fill = vl_status)) +
    geom_bar(stat = "identity", position = "stack") +
    #geom_bar(aes(y = -suppressed, fill = "suppressed"), stat = "identity") +
    geom_text(data = df_vl_age %>% filter(vl_status == "suppressed"),
              aes(y = 2, label= paste0(round(value, digits = 0), "%")),
